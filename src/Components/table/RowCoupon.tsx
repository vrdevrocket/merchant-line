...
import { BLANK_IMAGE_URL, enumStatus, enumTypeFetchList, PATH_COUPON } from "@configs";
...
export const ComponentRowCoupon = (props: IProps) => {
    //hooks
    ...
    const handleDelete = async () => {
        if (data._id) {
            try {
                await couponAPI.delete(data._id);
    ...
    const handleDuplicate = async () => {
        if (data._id) {
            try {
                await couponAPI.duplicate(data._id);
    ...
    const handleUpdateStatus = async (e: boolean) => {
        if (data._id) {
            try {
                await couponAPI.toggleStatus(data._id, e ? enumStatus.ACTIVE : enumStatus.INACTIVE);
                callbackGetList(enumTypeFetchList.duplicate);
            ...
            <Row style={{ height: "100%" }} gutter={24} type="flex" align="middle">
                <Col className="col-item" lg={9} md={9} sm={24} xs={24}>
                    <Avatar
                        ...
                        className="avatar"
                    />
                    <Link className="visible-md" to={`${PATH_COUPON}/${data._id}`}>
                        <p className="user-name">{data.name}</p>
                    </Link>
                    <div className="item-info visible-ms">
                        <Link to={`${PATH_COUPON}/${data._id}`}>
                            <p className="user-name">{data.name}</p>
                        </Link>
