...
import { BLANK_IMAGE_URL, enumStatus, enumTypeFetchList, PATH_BENEFITS } from "@configs";
...
export const ComponentRowBenefit = (props: IProps) => {
    //hooks
    ...
    const handleDelete = async () => {
        if (data._id) {
            try {
                await benefitAPI.delete(data._id);
    ...
    const handleDuplicate = async () => {
        if (data._id) {
            try {
                await benefitAPI.duplicate(data._id);
            ...
            error(t("message.duplicate.fail"));
        }
    };
    ...
    const handleUpdateStatus = async (e: boolean) => {
        if (data._id) {
            try {
                await benefitAPI.updateStatus(data._id, {
    ...
    return (
        <div className="table-row table-row-large ">
            ...
            <Row style={{ height: "100%" }} gutter={24} type="flex" align="middle">
                <Col className="col-item" lg={7} md={7} sm={24} xs={24}>
                    <Avatar
                        shape="square"
                        src={data?.imageUrl[0] || BLANK_IMAGE_URL}
                        className="avatar"
                    />
                    <Link className="visible-md" to={`${PATH_BENEFITS}/${data._id}`}>
                        <p className="user-name">{data.name}</p>
                    </Link>
                    <div className="item-info visible-ms">
                        <Link to={`${PATH_BENEFITS}/${data._id}`}>
                            <p className="user-name">{data.name}</p>
                        </Link>
                        <p className="eng item-desc">
                ...
                <Col className="col-item visible-md" lg={5} md={5} sm={6} xs={6}>
                    <p className="eng">
                        {moment(new Date(data?.startDate || "")).format("DD/MM/YYYY HH:mm")}
                        <span> - </span>
                        {moment(new Date(data?.endDate || "")).format("DD/MM/YYYY HH:mm")}
